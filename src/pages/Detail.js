...
const Detail = ({ setActive, user }) => {
  ...
  const getBlogDetail = async () => {
    setLoading(true);
    const blogRef = collection(db, "blogs");
    const docRef = doc(blogRef, id); // Updated this line to correctly reference the document using the provided id
    try {
        const blogDetailSnapshot = await getDoc(docRef);
        if (blogDetailSnapshot.exists()) {
            ...
            // Document with provided id doesn't exist
